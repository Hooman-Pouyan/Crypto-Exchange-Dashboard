...
export class ExchangeService {
  ...
  updateData(updateData: any, apiEndpoint: string,network=false) {
    const header = new HttpHeaders(this.serviceconfigs.getApiHeaders())
    let apiUrl=network?this.serviceconfigs.apiUrl + apiEndpoint :this.serviceconfigs.apiUrl + apiEndpoint + updateData.id
  ...
  getCurrencyAccounts(queryParams: Object){
    const header = new HttpHeaders(this.serviceconfigs.getApiHeaders())
    const param = this.serviceconfigs.createQueryParams(queryParams)
    return this.http.get<any>(`${this.serviceconfigs.apiUrl}${Api_Endpoints.currency.address.show}`, {headers: header,params:param}).pipe(
      // takeUntilDestroyed()
    )
  }
  ...
  getCurrencySettings(queryParams: Object){
    const header = new HttpHeaders(this.serviceconfigs.getApiHeaders())
    const param = this.serviceconfigs.createQueryParams(queryParams)
    return this.http.get<any>(`${this.serviceconfigs.apiUrl}${Api_Endpoints.currency.setting.show}`, {headers: header,params:param}).pipe(
      // takeUntilDestroyed()
    )
  }
